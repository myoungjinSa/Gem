...
void GameObject::Render(Renderer& renderer, ShaderParameter& shaderParam) {
	...
	case Renderer::eRenderMode::SOLID:
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		break;
	case Renderer::eRenderMode::WIREFRAME:
...
bool Cube::Initialize(Renderer& renderer) {
	...
	meshBuilder.CopyToMesh(renderer, *meshes.back(), &OpenGL::BindBuffer, &Vertex::Copy, sizeof(Vertex));
	meshBuilder.End();
...
void Cube::Render(Renderer& renderer, ShaderParameter& shaderParam) {
	...
	renderer.SetDrawMode(Renderer::ePrimitive::TRIANGLES);
